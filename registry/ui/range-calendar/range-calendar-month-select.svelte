...
<span
	class={cn(
		"has-focus:border-ring rounded-md border-input has-focus:ring-ring/50 relative flex border shadow-xs has-focus:ring-[3px]",
		className
	)}
>
	<RangeCalendarPrimitive.MonthSelect bind:ref class="absolute inset-0 opacity-0" {...restProps}>
		{#snippet child({ props, monthItems, selectedMonthItem })}
			...
			<span
				class="[&>svg]:text-muted-foreground flex h-8 items-center gap-1 pr-1 pl-2 text-sm font-medium select-none [&>svg]:size-3.5"
				aria-hidden="true"
			>
