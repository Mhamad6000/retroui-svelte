<script lang="ts">
	...
	import * as Command from "$registry/ui/command/index.js";
	import * as Dialog from "$registry/ui/dialog/index.js";
	import { Input } from "$registry/ui/input/index.js";
	import * as Tooltip from "$registry/ui/tooltip/index.js";
				...
				<Tooltip.Trigger>
					{#snippet child({ props })}
						<Button
							...
							size="icon"
							variant="secondary"
							class="ml-auto size-8 rounded-full shadow-sm"
							onclick={() => (open = true)}
						>
							<PlusIcon />
	...
	<Card.Footer>
		...
		>
			...
			<Button
				...
				class="absolute shadow-sm right-2 top-1/2 size-6 rounded-full !-translate-y-1/2"
			>
